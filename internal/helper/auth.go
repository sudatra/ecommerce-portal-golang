...
func (a Auth) CreateHashedPassword(p string) (string, error) {
	if len(p) < 6 {
		return "", errors.New("password length should be atleast 6");
	}
...
func (a Auth) VerifyPassword(p string, h string) error {
	if len(p) < 6 {
		return errors.New("password length should be atleast 6");
	}
...
func (a Auth) VerifyToken(t string) (domain.User, error) {
	...
	if err != nil {
		return domain.User{}, errors.New("invalid sigining method")
	}
